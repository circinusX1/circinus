...
SerialPort::SerialPort():_port(0),_error(0),_wrdelay(0)
{
...
int  SerialPort::_writebybyby(unsigned char *buff, int len, int to)
{
...
resend:
            ...
            {
                LOGERR("... port failed to send byte: errno" << errno);
                return -1;
            }
        }
    }while(left>0 && ::tick_count() < ct);

    LOGDETAIL2("... port sent bytes="<< (len-left));
...
int SerialPort::Read(unsigned char *buf, int size, int to, int sto)
{
    int     nbytes = 0;
    struct  timeval tv;
    fd_set  fds;
    ...
    do{
        ...
        if(sel > 0 && FD_ISSET(_port, &fds))
        {
            ...
            {
                nbytes+=shot;
                LOGDETAIL2("... port received bytes="<< nbytes);
                continue;
            }
        }
        break;
...
int SerialPort::SendBuf(unsigned char *buf, int size, int to)
{
    ...
    do{
        ...
        if(n < 0)
        {
            if(errno == EAGAIN)
            {
                LOGERR("... port send EAGAIN");
                ::msleep(8);
                continue;
            }
            else
            {
                LOGERR("... port send error ");
                return -1;
            }
        }
        if( n > 0)
    ...
    LOGDETAIL2("... port sent bytes="<< (size-left));
    return left;
}
...
int SerialPort::Read(unsigned char *buf, int lsize, int to)
{
    int n=0;
    int toget = lsize;
    int got   = 0;
    ...
    while(::tick_count() < fut && toget>0)
    {
        ...
        {
            LOGERR("ReadFile: Port closed: " << GetLastError());
            return -1;
        }
        ::msleep(2);
    }
    ...
    if(got)
        LOGDETAIL2("<- port file-descriptor received:" << got <<" bytes\n");
    return got;
}
...
int SerialPort::SendBuf(unsigned char *buf, int bytes, int to)
{
    ...
    while(GetTickCount()<ct && left>0)
    {
        ...
        if(::WriteFile(_port, buf+written, left, (LPDWORD)((void *)&n), NULL)==FALSE)
        {
            ...
            {
                LOGERR("<- port file-descriptor error sending bytes:" << GetLastError());
                return -1;
            }
        }
        if(n>0)
        ...
        {
            LOGERR("<- port file-descriptor error sending bytes");
            return -1;
        }
    }

    if(left==0){
        LOGDETAIL2("<- port file-descriptor sent:" << bytes <<" bytes");
    }
    else{
        LOGERR("<- port file-descriptor error sending bytes");
    }
