...
public:
    ...
    bool Compile(SQObjectPtr &o)
    {
        ...
        SQMembState funcstate(_ss(_vm), NULL,ThrowError,this);
    ...
    void EmitLoadConstFloat(SQFloat value,int target)
    {
        ...
        if(sizeof(SQFloat) == sizeof(int32_t)) {
    ...
    //mco-mco types for bitwise
    template <typename T>void LocalDeclSt()
    {
        SQObject varname;
        Lex();
        if( _token == TK_FUNCTION) {
            ...
            Expect(_SC('('));
            CreateFunction(varname,false);
            _fs->AddInstruction2<T>(_OP_CLOSURE, _fs->PushTarget(),
        ...
        do {
            ...
            if(_token == _SC('=')) {
                Lex(); Expression();
                T src = static_cast<T>(_fs->PopTarget());
                T dest = static_cast<T>(_fs->PushTarget());
                if(dest != src) _fs->AddInstruction2<T>(_OP_MOVE, dest, src);
            }
            else{
                _fs->AddInstruction2<T>(_OP_LOADNULLS, _fs->PushTarget(),1);
            }
            _fs->PopTarget();
            _fs->PushLocalVariable(varname);
            if(_token == _SC(','))
    ...
    {
        SQMembState *funcstate = _fs->PushChildState(_ss(_vm));
