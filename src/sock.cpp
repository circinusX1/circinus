/**
# Copyright (C) 2007-2014 Chincisan Octavian-Marius
#
# Licensed under the Apache License, EmbVersion 2.0 (the "License");
...
SADDR_46 sock::sip2ip(const char* sip, uint16_t port)
{
    int a,b,c,d,e=port;
    ...
    if(strchr(sip,':') && 5==::sscanf(sip,"%u.%u.%u.%u:%d",&a,&b,&c,&d,&e))
    {
            ...
            c&=0xff;
            d&=0xff;
            return SADDR_46(u_int32_t(a<<24|b<<16|c<<8|d), (uint16_t)e);
    }
...
//-----------------------------------------------------------------------------
char* sock::GetLocalIP(const char* aleLOW)
{
...
//-----------------------------------------------------------------------------
int  sock::select_receive(char* buff, int length, int toutms, int wait)const
{
    return select_receive((unsigned char*) buff, length, toutms,  wait);
}

//-----------------------------------------------------------------------------
int  sock::select_receive(unsigned char* buff, int length, int toutms , int wait)const
{
    int      bytes = -1;
    ...
    if(sel > 0 && FD_ISSET(_thesock, &rdSet))
    {
        ...
        bytes = receive((char*)buff, length);
        if(0 == bytes)
        {
...
//-----------------------------------------------------------------------------
int tcp_sock::receive(char* buff, int length, int , const char* )const
{
     tcp_sock* ptr =  const_cast<tcp_sock*> (this);
    ptr->_io &= ~0x1;
    ptr->_error = 0;
...
bool    tcp_srv_sock::term(bool emptyit)
{
...
tcp_clis::tcp_clis(const tcp_clis& s):tcp_sock()
{
    _thesock    = s._thesock;
    _error      = s._error;
...
tcp_clis& tcp_clis::operator=(const tcp_clis& s)
{
    if(this != &s)
    {
        _thesock    = s._thesock;
...
bool tcp_clis::is_really_connected()
{
    if(_thesock>0)
    {
...
bool    tcp_clis::term(bool emptyit)
{
    if(!isopen())   return false;
    int         n = 0;
    _connecting = 0;
...
SOCKET tcp_srv_sock::create(int port, int opt, const char* iface)
{
    ...
    // option before bind will aleLOW recreation on same SIN when closed imediatelly
    if(opt)
    {
        int one = 1;
        ::setsockopt(_thesock, SOL_SOCKET, /*SO_REUSEADDR*/opt, (char *)&one, sizeof(one));
...
SOCKET tcp_srv_sock::accept(tcp_clis& cliSock)
{
    ...
    {
        LOGE("accept " << errno <<", " << strerror(errno));
        _error = errno;
        cliSock.term();
        return 0;
    }
...
SOCKET tcp_clis::create(const SADDR_46& r, int opt)
{
    return create(0,opt,0);
}

//-----------------------------------------------------------------------------
SOCKET tcp_clis::create(int , int opt, const char* iface)
{
    _hostent = 0;
    assert(_thesock<=0);
    _thesock = ::socket(AF_INET, SOCK_STREAM, 0);
...
int tcp_clis::try_connect(const char* sip, int port)
{
...
/*
int tcp_clis::raw_connect(const char* suip4, int port)
{
    if(_ttatoi(suip4))
    {
...
*/
int tcp_clis::raw_connect(u_int32_t ip4,  int port)
{
...
int tcp_clis::raw_connect(const SADDR_46&  saddr, int tout)
{
    _error   = 0;
    _connecting = 0;
    _error   = 0;
...
void      tcp_clis::raw_sethost(const SADDR_46& uip4)
{
    _remote_sin = uip4;
}

int     tcp_clis::raw_connect_sin()
{
    _error = 0;
    _connecting = 0;
    _error = 0;
...
//-----------------------------------------------------------------------------
int tcp_clis::i4connect(const SADDR_46&  addr, CancelCB cbCall, void* pUser)
{
    int             err;
    fd_set          fdWr;
...
//-----------------------------------------------------------------------------
int tcp_clis::s4connect(const char* sip, int port, CancelCB cbCall, void* pUser)
{
    int             err;
    fd_set          fdWr;
    timeval         tv = {1, 0};
...
//-----------------------------------------------------------------------------
int tcp_clis::connect(const char* sip, int port, CancelCB cbCall, void* pUser)
{
    _connecting = 1;
    _error = 0;
    sockaddr_in    locSin;
...
//-----------------------------------------------------------------------------
SOCKET udp_sock::create(const SADDR_46& r, int tcpp)
{
    _error    = 0;
    assert(_thesock<=0);
...
//-----------------------------------------------------------------------------
SOCKET udp_sock::create(int port, int tcpp, const char* addr)
{
    _error    = 0;
    assert(_thesock<=0);
    _thesock = ::socket(AF_INET, SOCK_DGRAM, tcpp);
