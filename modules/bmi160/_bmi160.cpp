...
/*!
 *  @brief This API is the enitry point for sensor.It performs
...
int8_t bmi160_init(struct bmi160_dev *dev)
{
    ...
    if (rslt == BMI160_OK)
    {
        ...
        dev->chip_id = 0;
        while ((itry--) && (dev->chip_id != BMI160_CHIP_ID))
        {
            /* Read chip_id */
...
double convert_raw(int raw, float range_abs)
{
// https://github.com/DanielXie00/esp32_bmi160/blob/master/main/main.c
    double val = (float)raw;
    double slope = (range_abs * 2.0) / 65535.0;
    return -(range_abs) + (slope * (val + 32768.0));
}
//sdf
...
int get_ar(double* a, double* g, bmi160_com_fptr_t r, bmi160_com_fptr_t w, bmi160_delay_fptr_t d)
{
...
/* To read both Accel and Gyro data along with time*/
	...
	a[0] = convert_raw(accel.x,2)*10.0; // from https://github.com/DanielXie00/esp32_bmi160/blob/master/main/main.c
	a[1] = convert_raw(accel.y,2)*10.0;
	a[2] = convert_raw(accel.z,2)*10.0;

    //printf("BMI160:   A: X:%04.04f Y:%04.04f Z:%04.04f   (m/s^2)\n",  ax,ay,az);
    g[0] = (gyro.x * 250.0) / 32768.0;
    g[1] = (gyro.y * 250.0) / 32768.0;
    g[2] = (gyro.z * 250.0) / 32768.0;

    //printf("BMI160:   R: U=%06.04f V=%06.04f W=%06.04f   (deg/sec^2)\n", gx,gy,gz);
